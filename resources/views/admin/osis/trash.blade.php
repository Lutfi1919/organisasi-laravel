...
<link rel="stylesheet" href="{{ asset(path: 'css/users.css')}}">
    ...
    <div class="container mb-5" style="margin-top: 100px; font-family: Unbounded;">
        ...
        <div class="d-flex justify-content-end gap-1">
            <a href="{{ route('admin.osis.index')}}" class="btn utility-btns btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
        </div>
                    ...
                    @foreach ($councils as $key => $council)
                        <tr class="fw-light">
                            ...
                            <td class="text-center action-btns">
                                <form action="{{ route('admin.osis.restore', $council['id'])}}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn btn-sm btn-outline-success utility-btns"><i class="bi bi-recycle"></i> Pulihkan Data</button>
                                </form>
                                <form action="{{ route('admin.osis.delete_permanent', $council['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
